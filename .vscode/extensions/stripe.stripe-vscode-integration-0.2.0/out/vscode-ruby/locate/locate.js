"use strict";
// Adapted from https://github.com/rubyide/vscode-ruby/blob/5c8f5970b081cdf1a3a3f98d155b28295b6dd2e7/src/locate/locate.js
...
function camelCaseRegExp(query) {
    ...
    return new RegExp(`[${prefix.toLowerCase()}${prefix.toUpperCase()}]${escaped
        .slice(1)
        .replace(/[A-Z]|([a-z])/g, (char, lower) => {
        if (lower)
...
class Locate {
    ...
    _query(query) {
        ...
        const nameMatches = filter(symbols, nameQuery, (symbolInfo, regexp) => {
            return (_.includes(segmentTypes, symbolInfo.type) &&
                regexp.test(symbolInfo.name));
        });
    ...
    parse(absPath) {
        const promise = new Promise((resolve, reject) => this._parseAsync(absPath));
        this.parseQueue.push(promise);
        return promise;
    }
    _parseAsync(absPath, relPath) {
        const rPath = relPath || path.relative(this.root, absPath);
        if (this.settings.exclude && minimatch(rPath, this.settings.exclude))
            return Promise.resolve();
        if (this.settings.include && !minimatch(rPath, this.settings.include))
            return Promise.resolve();
    ...
    walk() {
        ...
        }).then(() => console.log(`[ruby] locate: walk completed! (${new Date().getTime() -
            startTime}msec)`));
    ...
    _walkDir(dir, callback) {
        fs.readdir(dir, (err, files) => {
            ...
            files.forEach((file) => {
                this.walkQueue.push((cb) => this._walkNode(dir, file, cb));
            });
            callback();
        });
    }
        ...
        fs.stat(absPath, (err, stats) => {
            ...
            if (stats.isDirectory()) {
                if (!(this.settings.exclude && minimatch(relPath, this.settings.exclude))) {
                    this.walkQueue.push((cb) => this._walkDir(absPath, cb));
                }
                callback();
            }
            else {
